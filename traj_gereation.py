...
class TrajGenerator:
    ...
    def resample_path(self, path):
        ...
        for i in range(1, nfe):
            ...
            raw_v = math.sqrt(dx**2 + dy**2) / dt
            result.states[i].v = min(raw_v, self.max_velocity)
        ...
        if nfe > 0:
            ...
            for i in range(1, nfe):
                limit_v_accel = result.states[i-1].v + self.max_acceleration * dt
                result.states[i].v = min(result.states[i].v, limit_v_accel)
                
            result.states[-1].v = 0.0
            for i in range(nfe - 2, -1, -1):
                limit_v_decel = result.states[i+1].v + self.max_acceleration * dt
                result.states[i].v = min(result.states[i].v, limit_v_decel)
                
            result.states[0].omega = result.states[1].omega if nfe > 1 else 0.0
            result.states[-1].omega = result.states[-2].omega if nfe > 1 else 0.0
        # YOUR CODE ENDS HERE
        ...
        for i in range(1, nfe):
            ...
            result.states[i].a = dv / dt
            
        if nfe > 0:
            result.states[0].a = result.states[1].a if nfe > 1 else 0.0
        # YOUR CODE ENDS HERE
    ...
    def generate_optimal_time_profile_segment(self, stations, start_time):
        ...
        for i in range(len(stations) - 1):
            ...
            profile[i+1] = math.sqrt(profile[i]**2 + 2 * max_accel * ds)
        # YOUR CODE ENDS HERE
        ...
        for i in range(len(stations) - 1, accel_idx, -1):
            ...
            limit_v = math.sqrt(profile[i]**2 + 2 * max_accel * ds)
            profile[i-1] = min(profile[i-1], limit_v)
        # YOUR CODE ENDS HERE
        ...
        for i in range(len(profile)):
            ...
            profile[i] = min(profile[i], max_velocity)
        # YOUR CODE ENDS HERE
    ...
    def to_continuous_angle(self, angles):
        ...
        for i in range(1, len(angles)):
            ...
            diff = self.normalize_angle(angles[i] - continuous[-1])
            continuous.append(continuous[-1] + diff)
        return continuous
        # YOUR CODE ENDS HERE
