...
class Astar:
    def solve_astar(self, s, g, graph_vert_list, adjacency_matrix, edge_dict):
        ...
        while not open_set.empty():
            ...
            neighbors = self.get_neighbor(current, graph_vert_list, adjacency_matrix)
            for neighbor in neighbors:
                if neighbor in closed_set:
                    continue

                cost = self.cal_expand_cost(current, neighbor, edge_dict)
                if cost == np.inf:
                    continue 

                g_score = costs.get(current, np.inf) + cost

                if g_score < costs.get(neighbor, np.inf):
                    costs[neighbor] = g_score
                    parent_node[neighbor] = current
                    
                    f_score = g_score + self.calH(neighbor, g)
                    open_set.put((f_score, neighbor))
        
        for node in closed_set:
            if node[0] == g[0] and node[1] == g[1]:
                print(f"INFO: Cannot reach exact angle of goal {g}. Falling back to reached state {node}.")
                return self.traverse_path(s, node, parent_node)

        print("ERROR: A* failed to find any path to the goal.")
        return []   
        # YOUR CODE ENDS HERE
    ...
    def traverse_path(self, s, g, parent_node):
        ...
        while current != s:
            ...
            if current not in parent_node:
                print(f"Warning: Disconnected path at {current}")
                return []  
