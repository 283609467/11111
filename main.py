...
def main(args=None):
    ...
    if path:
        ...
        plt.plot(y, x, color='green', linewidth=2.0, label='Lattice Path')
        
        if path_rrt:
            rrt_x = [p[0] / 3.0 for p in path_rrt]
            rrt_y = [p[1] / 3.0 for p in path_rrt]
            plt.plot(rrt_x, rrt_y, color='red', linewidth=2.0, label='RRT Path')

        if path_prm:
            prm_x = [p[0] / 3.0 for p in path_prm]
            prm_y = [p[1] / 3.0 for p in path_prm]
            plt.plot(prm_x, prm_y, color='blue', linewidth=2.0, label='PRM Path')

        plt.title('Path Planners: Collision-Free Paths')
        plt.legend()
